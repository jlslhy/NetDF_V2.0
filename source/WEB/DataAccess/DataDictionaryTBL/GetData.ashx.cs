...
namespace WEB.DataAccess.DataDictionaryTBL
{
    ...
    /// GetData 的摘要说明
    /// </summary>
    public partial class GetData : GetDataBase, IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void GetDataList()
        {
            ...
            string condition = MakeConditionString<DBControl.DBInfo.Tables.DataDictionary>(HttpContext.Current, "s_");
            ...
            try
            {
                ...
                switch (selectType)
                {
                    case enumSelectType.FirstPage:
                        idr = DBControl.Base.DBAccess.GetFirstPageDataIDR<DBControl.DBInfo.Tables.DataDictionary>(fieldArr.ToString(","), _descOrder, _pageSize, condition);
                        break;
                    case enumSelectType.LastPage:
                        idr = DBControl.Base.DBAccess.GetLastPageDataIDR<DBControl.DBInfo.Tables.DataDictionary>(fieldArr.ToString(","), _descOrder, _pageSize, condition);
                        break;
                    case enumSelectType.PrevPage:
                        idr = DBControl.Base.DBAccess.GetPrevPageDataIDR<DBControl.DBInfo.Tables.DataDictionary>(fieldArr.ToString(","), _descOrder, _pageSize, _minid, _maxid, condition);
                        break;
                    case enumSelectType.NextPage:
                        idr = DBControl.Base.DBAccess.GetNextPageDataIDR<DBControl.DBInfo.Tables.DataDictionary>(fieldArr.ToString(","), _descOrder, _pageSize, _minid, _maxid, condition);
                        break;
                    case enumSelectType.GoToPage:
                        idr = DBControl.Base.DBAccess.GetPageDataIDR<DBControl.DBInfo.Tables.DataDictionary>(fieldArr.ToString(","), _descOrder, _pageSize, _page, condition);
                        break;
                ...
                if (jArray.Count > 0)
                {
                    ...
                    if (IsGetTotal)
                    {
                        _total = DBControl.Base.DBAccess.Count<DBControl.DBInfo.Tables.DataDictionary>(condition);
                    }
