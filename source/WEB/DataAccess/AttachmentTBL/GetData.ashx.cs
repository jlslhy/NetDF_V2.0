...
namespace WEB.DataAccess.AttachmentTBL
{
    ...
    public class GetData : GetDataBase, IHttpHandler,System.Web.SessionState.IRequiresSessionState
    {
        ...
        DBControl.DBInfo.Tables.Attachment tableInfo = new DBControl.DBInfo.Tables.Attachment();
        public void ProcessRequest(HttpContext context)
        {
            /********  初始化 Start  ********/
            _tableName = "Attachment";
            _orderBy = "AttachID asc";
            _idField = "AttachID";
            ...
            try
            {
                ...
                switch (selectType)
                {
                    case enumSelectType.FirstPage:
                        idr = DBControl.Base.DBAccess.GetFirstPageDataIDR<DBControl.DBInfo.Tables.Attachment>(fieldArr.ToString(","), _descOrder, _pageSize, condition);
                        break;
                    case enumSelectType.LastPage:
                        idr = DBControl.Base.DBAccess.GetLastPageDataIDR<DBControl.DBInfo.Tables.Attachment>(fieldArr.ToString(","), _descOrder, _pageSize, condition);
                        break;
                    case enumSelectType.PrevPage:
                        idr = DBControl.Base.DBAccess.GetPrevPageDataIDR<DBControl.DBInfo.Tables.Attachment>(fieldArr.ToString(","), _descOrder, _pageSize, _minid, _maxid, condition);
                        break;
                    case enumSelectType.NextPage:
                        idr = DBControl.Base.DBAccess.GetNextPageDataIDR<DBControl.DBInfo.Tables.Attachment>(fieldArr.ToString(","), _descOrder, _pageSize, _minid, _maxid, condition);
                        break;
                    case enumSelectType.GoToPage:
                        idr = DBControl.Base.DBAccess.GetPageDataIDR<DBControl.DBInfo.Tables.Attachment>(fieldArr.ToString(","), _descOrder, _pageSize, _page, condition);
                        break;
