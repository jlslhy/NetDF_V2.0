...
function UploadInitial() {
    ...
    uploader.on('uploadSuccess', function (file, response) {
        if (response.chunked) {
            $.post("MergeFiles.ashx", { guid: GUID, fileExt: response.f_ext, fileName: file.name },
            function (data) {
                data = $.parseJSON(data);
                if (data.hasError) {
                    alert('文件合并失败！');
                    ...
                    UploadSuccessProcess(data);
                }
            });
        } else { // add by LHJ 2017-05-15
            // var obj;
            // obj.batchGUID = response.batchGUID;
            UploadSuccessProcess(response);
        }
    });
