...
function ReqStr(url, par) {
    url = $.trim(url);
    var parValue = "";
    var startIndex = url.indexOf("?" + par + "=");
    if (startIndex < 0) {
        startIndex = url.indexOf("&" + par + "=");
    }
    if (startIndex < 0) {
...
function ShowBox(titleText, width, height, top) {
    if (typeof ($("#CoverDiv")[0]) == 'undefined') {
        ...
        $(CoverDiv).css({ "position": "absolute", "top": "0px", "width": "100%", "z-index": "101", "display": "none", "background-color": "#BBD79F" });
...
function ShowCoverDiv() {
    if (typeof ($("#CoverDiv")[0]) == 'undefined') {
        ...
        $("body").append(CoverDiv);
        $(CoverDiv).attr("id", "CoverDiv");
        $(CoverDiv).css({ "position": "absolute", "top": "0px", "width": "100%", "z-index": "100", "display": "none", "background-color": "#86B7E1" });
        $(CoverDiv).height($(document).height());
    }
