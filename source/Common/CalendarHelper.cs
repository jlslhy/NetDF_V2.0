...
namespace Common
{
    ...
    {
        private static ChineseCalendar _calendar = new ChineseCalendar(DateTime.Now);
        private static string[] _monthString =
                {
                    "出错","一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"
                };
        private static string[] _quarterString = { "出错", "一季度", "二季度", "三季度", "四季度" };
        /// <summary>
        /// 中文日历
        /// </summary>
        public static ChineseCalendar ChsCalendar
        {
            get
            {
        ...
        /// <returns></returns>
        public static string GetChsQuarter(int quarter)
        {
            return _quarterString[quarter];
        ...
        /// <param name="quarter"></param>
        /// <returns></returns>
        public static string GetChsMonth(int month)
        {
        ...
        /// <summary>
        /// 从第一个星期天开始计算,指定日期是指定日期所在月份的第几周,如果不属于第一周以上，则返回0
        /// </summary>
        /// <param name="day">日期</param>
        /// <returns></returns>
        public int GetWeekNo2(DateTime dtime)
        {


            DateTime firstDayOfMonth = new DateTime(dtime.Year, dtime.Month, 1);
            int firstDayWeek = (int)firstDayOfMonth.DayOfWeek;
            int days = dtime.Day + (firstDayWeek.Equals(0) ? 7 : firstDayWeek) - 7;
            if (days <= 0)
            {
                return 0;
            }
        ...
        /// <summary>
        /// 从第一个星期天开始计算,指定日期是指定日期所在月份的第几周
        /// </summary>
        /// <param name="day">日期</param>
        /// <returns></returns>
        public static int GetWeekNo(DateTime day)
        {
        ...
        public static int GetWeekNo(DateTime day, out DateTime mondayDate)
        {
            string y = day.Year.ToString();
            string m = day.Month.ToString();
            string s = y + "-" + m + "-1";
            ...
            if (w == 0)
            {
                ...
                w = GetWeekNo(dt.AddSeconds(-1), out mondayDate);
            }
        ...
        /// <summary>
        /// 从第一个星期天开始计算,指定日期是指定日期所在月份的第几周的月份
        /// </summary>
        /// <param name="day">日期</param>
        /// <returns></returns>
        public static int GetWeekNoInMonth(DateTime day)
        {
            string y = day.Year.ToString();
            string m = day.Month.ToString();
            string s = y + "-" + m + "-1";
        ...
        /// <summary>
        /// 从第一个星期天开始计算,指定日期是指定日期所在月份的第几周的星期日的日期
        /// </summary>
        /// <param name="day">日期</param>
        /// <returns></returns>
        public static DateTime GetWeekNoStartDate(DateTime day)
        {

            DateTime st = day;

            GetWeekNo(day, out st);
        ...
        {
            DateTime dt = GetFirstDayOfQuarter(year, quarter);
            return dt.AddMonths(3).AddSeconds(-1).Date;
        }


        /// <summary>
        /// 那年那月共有几个周 
        /// add by 朱庆勇 由 年 月 算本月有几周
        /// </summary>
        /// <param name="year"></param>
        /// <param name="month"></param>
        /// <returns></returns>
        public static int GetMonthHowWeek(int year, int month)
        {
            int res = 0;
            if (year == 0 && month == 0)
            {
            ...
            //周日GetHashCode的为0
            int startweeks = (int)monthStart.DayOfWeek; //1号是星期几
            int days = DateTime.DaysInMonth(year, month); //当月有多少天
            int weekday;
            if (startweeks == 0)
                weekday = days;
            else
                weekday = days - 7 + startweeks;
            res = weekday / 7;  //取整
            if (weekday % 7 > 0) //取余
                res += 1;
            return res;
        ...
        /// <summary>
        /// 那看那月共第几周的周日的日期
        /// add by 朱庆勇 由 年 月 算周推日期
        /// </summary>
        /// <param name="year">年</param>
        /// <param name="month">月</param>
        /// <param name="week">周</param>
        /// <returns>DateTime?</returns>
        public static DateTime GetWeekOfSunday(int year, int month, int week)
        {
            int mw = GetMonthHowWeek(year, month); //本月有几个周
            DateTime monthStart = new DateTime(year, month, 1);
            //周日startweeks的为0
            int startweeks = (int)monthStart.DayOfWeek; //1号是星期几
            //计算本月第一个周日是本月的第几天 第一天为0开始
            int intfirstsumday = 0;
            if (startweeks > 0)
                intfirstsumday = 7 - startweeks;
            //定位参数中的星期天到第本月第一天的相差的天数
            int spanDays = (week - 1) * 7 + intfirstsumday;
            DateTime res = monthStart.AddDays(spanDays);
            return res;
        }
    ...
    /// 以下为使用的样例
    /// ChineseCalendar Cl=new ChineseCalendar(DateTime.Now)
    /// cl.ToString();
    /// </remarks>
    public class ChineseCalendar
    {
        ...
        public override string ToString()
        {
            ...
            str.Append(this.DateString + "(" + this.WeekDay + ")");
            str.Append(this.ChineseDateString);
            str.Append("(" + this.GanZhiYearString + ")");
            str.Append(" " + this.ChineseCalendarHoliday);
            str.Append(" " + this.DateHoliday);
            str.Append(" " + this.ChineseTwentyFourDay);
