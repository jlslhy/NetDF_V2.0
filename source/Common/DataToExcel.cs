...
namespace Common
{
    ...
    public class DataToExcel
    {
        ...
        #endregion
        public void CreateExcel(System.Data.DataTable dt, string FilePath)
        {
        ...
        public bool SaveExcelFile(Excel.Application excel, string filePath)
        {

            string fileNameIndex = DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss");
            return SaveExcelFile(excel, filePath, string.Format("file{0}", fileNameIndex));

        }


        public bool SaveExcelFile(Excel.Application excel, string filePath, string fileName)
        {
            bool success = false;
            ...
            switch (ExistsRegedit())
            {
                case 1:
                    {
                        excel.ActiveWorkbook.SaveAs(filePath + fileName, XlFileFormat.xlExcel9795, null, null, false, false, Excel.XlSaveAsAccessMode.xlNoChange, null, null, null, null, null);
                        break;
                    }
                case 2:
                    {
        ...
        public int ExistsRegedit()
        {
            ...
            if (f03 != null)
            {
                //string file03 = f03.GetValue("Path").ToString();             //if (File.Exists(file03 + "Excel.exe"))  ifused += 1;               
                ifused = 1;
            }
            ...
            if (f07 != null)
            {
                //string file07 = akey.GetValue("Path").ToString();             //if (File.Exists(file07 + "Excel.exe")) ifused += 2;                
                ifused = 2;
            }
